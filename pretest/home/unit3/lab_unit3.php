...
$sql = "SELECT chapter_3_status, Unit3 FROM users WHERE username = ?";
...
$unit3_score = $row['Unit3']; // คะแนน Unit1 ปัจจุบัน

// ตรวจสอบคำตอบที่ส่งมาทาง POST
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["answer"])) {
    $user_answer = trim($_POST["answer"]);
    $correct_answer = "MFA"; // กำหนดคำตอบที่ถูกต้อง

    if ($user_answer === $correct_answer) {
        // ถ้าผู้ใช้ตอบถูกต้อง ให้บันทึก chapter_1_status เป็น "completed"
        $update_sql = "UPDATE users SET chapter_3_status = 'completed' WHERE username = ?";
        ...
        if ($unit3_score > 0) {
            $new_unit3_score = $unit3_score - 1;
            $update_sql = "UPDATE users SET Unit3 = ? WHERE username = ?";
            $stmt = $conn->prepare($update_sql);
            $stmt->bind_param("is", $new_unit3_score, $logged_in_user);
            $stmt->execute();
            $stmt->close();
        }

        echo json_encode(["status" => "incorrect", "unit3" => $new_unit3_score ?? $unit3_score]);
        exit();
    }
}
...
<head>
    ...
    <title>Lab Unit 3: Passwords & Authentication</title>
...
<div class="nav">
    ...
    <section class="content-section">
        ...
        <div class="content-text">
            <p><strong>การพิสูจน์ตัวตนแบบหลายปัจจัย มีชื่อย่อว่าอะไร</strong></p>
