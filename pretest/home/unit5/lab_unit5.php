...
$sql = "SELECT chapter_5_status, Unit5 FROM users WHERE username = ?";
...
$unit5_score = $row['Unit5']; // คะแนน Unit1 ปัจจุบัน
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["answer"])) {
    ...
    if ($user_answer === $correct_answer) {
        ...
        $update_sql = "UPDATE users SET chapter_5_status = 'completed' WHERE username = ?";
        ...
        if ($unit5_score > 0) {
            $new_unit5_score = $unit5_score - 1;
            $update_sql = "UPDATE users SET Unit5 = ? WHERE username = ?";
            $stmt = $conn->prepare($update_sql);
            $stmt->bind_param("is", $new_unit5_score, $logged_in_user);
            $stmt->execute();
            $stmt->close();
        }

        echo json_encode(["status" => "incorrect", "unit1" => $new_unit5_score ?? $unit5_score]);
        exit();
    }
}
