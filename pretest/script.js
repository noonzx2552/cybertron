const questions = [
    ...
    function nextQuestion() {
        ...
        // เช็คคำตอบว่าเลือกถูกหรือผิด
        if (selectedOption.textContent === currentQuestion.answer) {
            score++;
            selectedOption.classList.add("correct");  // เพิ่มสีเขียวให้ตัวเลือกที่ตอบถูก
            playSound('correct');  // เล่นเสียงตอบถูก
        } else {
            selectedOption.classList.add("incorrect");  // เพิ่มสีแดงให้ตัวเลือกที่ตอบผิด
            options.forEach(option => {
                if (option.textContent === currentQuestion.answer) {
                    option.classList.add("correct");  // เพิ่มสีเขียวให้ตัวเลือกที่ถูกต้อง
                }
            });
            playSound('incorrect');  // เล่นเสียงตอบผิด
        }
    
        // ปิดการเลือกตัวเลือกใหม่เมื่อคำถามถูกตอบแล้ว
        options.forEach(option => {
            option.style.pointerEvents = 'none';  // ปิดการคลิกตัวเลือกหลังจากตอบ
        });
    
        currentQuestionIndex++;
        if (currentQuestionIndex < questions.length) {
            setTimeout(() => {
                loadQuestion();  // โหลดคำถามถัดไปหลังจากรอ 1 วินาที
                options.forEach(option => {
                    option.style.pointerEvents = 'auto';  // เปิดการคลิกตัวเลือกใหม่เมื่อคำถามโหลดใหม่
                });
            }, 1000);  // รอ 1 วินาทีเพื่อแสดงผลลัพธ์ก่อนที่จะไปคำถามถัดไป
        } else {
            showResult();
        }
    }
    ...
    function playSound(type) {
        ...
        if (type === 'correct') {
            audio.src = 'XCorrect.mp3';  // เสียงตอบถูก
        } else if (type === 'incorrect') {
            audio.src = 'Xwrong.mp3';  // เสียงตอบผิด
        }
        audio.play();
    }
